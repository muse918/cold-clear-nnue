...
impl<const O: usize, const MIN: i32, const MAX: i32> EncodeLayer<O, MIN, MAX> {
    ...
    fn convolute<const M: usize, const N: usize>(field: &[[bool; 10]; 40]) -> [[i32; 10]; 24] {
        let board_low: [[bool; 10]; 24] = field[..24].try_into().unwrap();
        let mut ret = board_low.map(|x| x.map(|y| y as i32));
...
lazy_static! {
    static ref MODEL: Nnue =
        read_nnue("C:\\Users\\Mingyu\\Downloads\\net_height_2x_2.net".to_string());
}
